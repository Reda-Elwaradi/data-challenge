...
def main(model_name=MODEL_NAME, model_retrieve_path=MODEL_RETRIEVE_PATH, dropout_rate=DROPOUT_RATE, max_epochs=MAX_EPOCHS, epoch_start=EPOCH_START, early_stopping_patience=EARLY_STOPPING_PATIENCE, total_batch_size=TOTAL_BATCH_SIZE, lr=LR, weight_decay=WEIGHT_DECAY, differential_lr=DIFFERENTIAL_LR, model_save_path=MODEL_SAVE_PATH, unfreeze_blocks=UNFREEZE_BLOCKS, unfreeze_epoch=UNFREEZE_EPOCH, scheduler_fn=SCHEDULER_FN, image_perturbations=IMAGE_PERTURBATIONS):
    ...
    best_epoch, best_val_loss = train_main(model_name, model_retrieve_path, dropout_rate, max_epochs, epoch_start, early_stopping_patience, total_batch_size, lr, weight_decay, differential_lr, model_save_path, unfreeze_blocks, unfreeze_epoch, scheduler_fn, image_perturbations)
    print("Entraînement terminé. Début du test...")
    test_main(model_name, model_save_path)
